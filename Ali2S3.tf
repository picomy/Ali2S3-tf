...
#- Step 0: Upload the Ali2S3 configuration file to the buckets
...
resource "aws_s3_bucket_object" "Ali2S3-conf" {
  bucket = aws_s3_bucket.confBucket.bucket
  key    = "Ali2S3.conf"
  source = "./Ali2S3.conf"
  etag   = filemd5("./Ali2S3.conf")
}
...
resource "aws_ecr_repository" "Ali2S3" {
  name = lower(join("-", ["cbdt", replace(var.DataSetName, "_", "-"), "Ali2S3"]))

  provisioner "local-exec" {
    working_dir = ".terraform/modules/Ali2S3/Ali2S3-Docker"
  ...
  provisioner "local-exec" {
    working_dir = ".terraform/modules/Ali2S3/Ali2S3-Docker"
    command     = "docker build -t ${aws_ecr_repository.Ali2S3.name} ."
  }

  provisioner "local-exec" {
    working_dir = ".terraform/modules/Ali2S3/Ali2S3-Docker"
    command     = "docker tag ${aws_ecr_repository.Ali2S3.name}:latest ${data.aws_caller_identity.current.account_id}.dkr.ecr.cn-north-1.amazonaws.com.cn/${aws_ecr_repository.Ali2S3.name}:latest"
  }

  provisioner "local-exec" {
    working_dir = ".terraform/modules/Ali2S3/Ali2S3-Docker"
    command     = "docker push ${data.aws_caller_identity.current.account_id}.dkr.ecr.cn-north-1.amazonaws.com.cn/${aws_ecr_repository.Ali2S3.name}:latest"
  }
}
...
data "aws_ecr_image" "Ali2S3-image" {
  repository_name = aws_ecr_repository.Ali2S3.name
  image_tag       = "latest"
}
...
resource "aws_ecs_task_definition" "Ali2S3" {
  family                   = lower(join("-", ["cbdt", replace(var.DataSetName, "_", "-"), "Ali2S3"]))
  ...
  container_definitions = jsonencode([
    {
      name      = lower(join("-", ["cbdt", replace(var.DataSetName, "_", "-"), "Ali2S3-c1"]))
      image     = join(":", [aws_ecr_repository.Ali2S3.repository_url, "latest"])
      essential = true
      "environment" : [
        {
          "name" : "SecretName",
          "value" : aws_secretsmanager_secret.Secret-Ali2S3.name
        },
        {
          "name" : "JobConf",
          "value" : "${aws_s3_bucket.confBucket.bucket}/${aws_s3_bucket_object.Ali2S3-conf.key}"
        },
        {
          "name" : "Region",
      ...
      "logConfiguration" : {
        ...
        "options" : {
          ...
          "awslogs-stream-prefix" : "${var.DataSetName}-Ali2S3"
        }
      }
    }
  ])
...
resource "aws_secretsmanager_secret" "Secret-Ali2S3" {
  name                    = lower(join("-", ["cbdt", replace(var.DataSetName, "_", "-"), "secret-Ali2S3"]))
  recovery_window_in_days = 0
}


variable "Ali2S3_keys" {
  default = {
    ali_access_key        = "Replace Me using your Ali access key"
    ali_secret_access_key = "Replace Me using your Ali secret key"
    s3_access_key_id      = "Replace Me using your aws access key"
    s3_secret_access_key  = "Replace Me using your aws access secret key"
  }
  type = map(string)
}


resource "aws_secretsmanager_secret_version" "Secret-Ali2S3-version" {
  secret_id     = aws_secretsmanager_secret.Secret-Ali2S3.id
  secret_string = jsonencode(var.Ali2S3_keys)
}
...
data "aws_ecs_task_definition" "Ali2S3" {
  task_definition = aws_ecs_task_definition.Ali2S3.family
}
...
resource "aws_cloudwatch_event_rule" "TriggerAli2S3Task-Rule" {
  name                = lower(join("-", ["cbdt", replace(var.DataSetName, "_", "-"), "Trigger-Ali2S3-Rule"]))
  description         = "Trigger the ECS Task to transfer data between src S3 and dst S3"
  schedule_expression = "cron(0 0 * * ? *)"
}


resource "aws_cloudwatch_event_target" "TriggerAli2S3Task-Rule-Target" {
  target_id = lower(join("-", ["cbdt", replace(var.DataSetName, "_", "-"), "cloudwatch-Ali2S3-target"]))
  arn       = aws_ecs_cluster.ECS-DT-Clu.arn
  role_arn  = aws_iam_role.CW-Invoke-ECS-Role.arn
  rule      = aws_cloudwatch_event_rule.TriggerAli2S3Task-Rule.name
  ecs_target {
    task_count          = 1
    task_definition_arn = aws_ecs_task_definition.Ali2S3.arn
    launch_type         = "FARGATE"
